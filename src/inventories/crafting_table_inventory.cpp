#include "crafting_table_inventory.h"

#include "player_inventory.h"
#include "core/server.h"
#include "core/utils.h"

void CraftingTableInventory::copyPlayerInventory(const PlayerInventory& playerInventory) {
    this->slots.clear();
    for (int i = 10; i <= 45; i++) {
        if (playerInventory.slots.contains(i - 1)) {
            this->slots[i] = playerInventory.slots.at(i - 1);
        }
    }
}

void CraftingTableInventory::copyToPlayerInventory(PlayerInventory& playerInventory) {
    playerInventory.slots.clear();
    for (int i = 10; i <= 45; i++) {
        if (this->slots.contains(i)) {
            playerInventory.slots[i - 1] = this->slots.at(i);
        }
    }
}


void CraftingTableInventory::HandleInventoryClick(
    uint8_t windowId,
    int32_t stateId,
    int16_t slotClicked,
    int8_t button,
    ...
    // Check window ID. If 0, dealing with player inventory.
    if (windowId == 0) {
        logMessage("Received inventory click for Player Inventory", LOG_DEBUG);
        return;
    }
    ...
    switch (mode) {
        ...
        case 1: {
            ...
            if (CraftableRecipe() != -1 && slotClicked == 0) {
                do {
    ...
    // If crafting slots changed (slots 1-9), update the result slot (0)
    if (std::ranges::any_of(changedSlotsFromClient, [](const auto &pair) {
        return pair.first >= 1 && pair.first <= 9;
    })) {
    ...
    // For simplicity:
    if (needFullResync) {
        SendFullInventoryUpdate(windowID);
    } else {
        ...
        for (auto & [clientSlot, clientData] : changedSlotsFromClient) {
            ...
            if (serverData.itemId != clientData.itemId || serverData.itemCount != clientData.itemCount) {
                // Send correction
                SendSlotUpdate(clientSlot, serverData, windowID);
            }
            // Also check if crafting result slot is different
            if (clientSlot >= 1 && clientSlot <= 9) {
                SlotData &resultData = getSlotData(0);
                SendSlotUpdate(0, resultData, windowID);
            }
        }
...
void CraftingTableInventory::moveItemToOtherInventorySection(int slotIndex) {
    ...
    // If clicked slot in main inventory or hotbar (10-45), move to crafting slots (1-9)
    if (slotIndex >= 10 && slotIndex <= 45) {
        startRange = 1; endRange = 9;
    } else if (slotIndex >= 1 && slotIndex <= 9) {
        startRange = 10; endRange = 45;
    } else if (slotIndex == 0) {
